...
*   **Параллельная разработка:** Вы можете начать писать код клиента еще до того, как API бэкенда будет реально реализовано.
...
*   `src/main/java/org/gulash/`:
    ...
    *   `UserClient.java`: HTTP-клиент (на базе Java `HttpClient`), который мы тестируем.
...
1.  **Откройте в IntelliJ IDEA:** Выберите корень проекта (файл `build.gradle`).
2.  **Запуск тестов:** Нажмите правой кнопкой на папку `src/test/java` -> **Run 'Tests in...'**.
3.  **Логи:** В консоли IDEA вы увидите подробные логи WireMock, которые показывают, какие запросы пришли и какие стабы сработали (или почему не сработали).